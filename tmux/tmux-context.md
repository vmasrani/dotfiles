...
- **SSH vs local branching**: `.tmux.conf` uses `if-shell '[ -n "$SSH_CLIENT" ]...'` twice — once for powerkit theme path, once for the plugins list. SSH sessions include `gpu_status.sh` but omit battery; local sessions include battery and weather. Both sessions share a single `host_status.sh` widget (user@host, plus the SSH client IP when remote) instead of separate ssh/user pills.
