...
2. **Claude's memory system.** That's under `~/.codex/projects/<proj>/memory/` and captures *user preferences* ("user prefers uv over pip") and *session feedback*. Notes here capture *engineering truths about the code*.
...
- **Output is always `./.notes/`**, never `~/.codex/...`. This is project-scoped, not global.
...
|----------------------|--------------------------------------|----------------------------------------|
| Location             | `~/.codex/projects/<proj>/memory/`  | `./.notes/` in project cwd             |
