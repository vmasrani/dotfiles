...
- User preferences / feedback ("user prefers X") -> `~/.codex/projects/<proj>/memory/`
...
Use the template at `~/.codex/skills/update-notes/assets/note-template.md` as the starting shape. When uncertain which category fits, read `~/.codex/skills/update-notes/references/category-seed.md`.
...
1. **Output path is always `./.notes/`** under the user's cwd. Never `~/.codex/...`.
