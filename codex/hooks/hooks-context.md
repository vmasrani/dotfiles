# hooks
> Claude Code lifecycle hooks: safety guards on tool use, TTS completion announcements, and JSON event logging to `.codex/logs/`.
`10 files | 2026-07-20`
...
- All hooks read JSON from stdin (Claude SDK contract) and log to `.codex/logs/` relative to `cwd` at time of invocation, not relative to the hook file's location.
...
- Hooks write logs to `Path.cwd() / '.codex/logs'` — this is the project CWD when Claude runs, not `~/.codex/`. Each project gets its own log directory under its own `.codex/logs/`.
...
- Hook scripts are symlinked into `~/.codex/hooks/` by `setup.sh`; changes must be made here in `codex/hooks/`, never in the symlink targets.
