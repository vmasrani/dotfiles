...
Some projects have `*-context.md` files for progressive disclosure. Each lives in the directory it describes and is generated by `/research`. These capture non-obvious conventions, gotchas, and key entry points — things you can't discover by reading code alone.

**MUST**: Before modifying files in any directory you haven't worked in yet this conversation, run `ctx-index . --depth 1` first. This is your default first step for orientation — skip it only when the user points you at a specific file or the change is a single targeted edit in a known file.

**Three-pass protocol (cheapest first):**
1. `ctx-index . --depth 1` — top-level map (~50 tokens). Identify which 1-3 dirs matter.
2. `ctx-peek {dir} 8` — key files and conventions (~200 tokens/dir). Find specific files.
3. Read the full `*-context.md` file only when you need gotchas, entry points, or non-obvious details.
