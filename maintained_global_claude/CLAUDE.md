...
- **This box (Linux VM: 64 threads, 503 GB RAM, ext4 — no reflink/CoW) has NO `cargo-nextest` and, as of the last check, NO sccache** (run `install_sccache` via `setup.sh` before any Rust work). A nextest command failing with "no such subcommand" is the CORRECT outcome: never silently rewrite it to `cargo test` and report success, and never read `cargo test` output with nextest semantics. `QUEUE_SLOTS` defaults to 1 everywhere; whether this box can safely support more is unmeasured — don't raise it without measuring peak RSS first. Queue flags, `--solo`, slots, coalescing, SJF, and the settled don't-re-investigate list: `~/dotfiles/maintained_global_claude/queue-reference.md`.
...
- **Git cleanup is local-only** — never delete remote branches or unmerged work unless told.
...
- **`just ci-fast` green locally before any PR whose diff touches what the gate exercises** (source, tests, lockfiles, build recipes) — never open a red PR. Skip only when the whole diff is outside the gate's graph (pure CI YAML, docs, comments); unsure means run it.
