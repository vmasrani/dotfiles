...
- **Concurrent work = pre-dev integration.** Whenever 2+ agents work 2+ issues in the same repo at once, nobody runs gates on their own branch: the orchestrator branches `pre-dev` from `dev` (own worktree, pushed) before dispatch; workers branch from `dev` as usual, keep only instant static checks, never run `ci-fast`/`ci-deep`/`test*`/`cargo test`/`nextest`/`queue`d gates, skip per-issue PRs, and on done rebase onto `pre-dev` and `git merge --no-ff` into it (`merge #N: <title>`), push, report. Orchestrator runs ONE `queue just ci-fast` on `pre-dev` after all merges; red → attribute per issue via the merge commits, route to the owner, re-merge, ONE gate again — never a second gate in flight. Green → ONE PR `pre-dev → dev` (`Closes #a, #b, …`) merged by `gh pr merge --merge`. Then mandatory cleanup (see the Git-cleanup bullet above): delete `pre-dev` and merged worker branches locally+remote, remove worktrees, `git worktree prune`, verify with `git worktree list`/`git branch -a`. Guard: while `refs/heads/pre-dev` exists, the heavy-guard hook denies gates (even correctly `queue`d ones) from any branch but `pre-dev`. Solo work is unchanged (see next bullet).
