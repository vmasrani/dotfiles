...
1. **Index** (~800 tokens for full project): Run `ctx-index .` via Bash.
   One line per directory — path and summary. Identify which 1-3 dirs matter.
...
**Rules:**
- **Start every task with `ctx-index .`** — even if you think you know where to look. It costs ~800 tokens and prevents wrong-directory mistakes.
