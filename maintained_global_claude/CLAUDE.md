...
- When exploring an **unfamiliar codebase** or directory for the first time
- When you need to understand project-specific conventions before making changes
- NOT when the user points you at a specific file or asks a simple question

**How to use (three-pass, cheapest first):**
...
3. Read the full context file only when you need gotchas, entry points, or non-obvious details.

**Rules:**
- Never load more than 2-3 full context files at once.
- If no context files exist, fall back to `ctx-tree` and direct exploration.
- Generate context files with `/research`. Clear them with `ctx-reset`.
