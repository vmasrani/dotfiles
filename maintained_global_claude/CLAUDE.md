...
Projects use `*-context.md` files for progressive disclosure. Each lives in the directory it describes and is generated by `/research`.

**Three-pass consumption (always follow this order):**

1. **Index** (cheapest): Run `ctx-index . --depth 1` via Bash.
   Top-level directories only — path and summary. Identify which 1-3 dirs matter.
   Then drill into a specific subtree with `ctx-index {dir}` if needed.

2. **Peek** (~200 tokens/file): Run `ctx-peek {dir} 8` on relevant dirs.
   Shows Key Files and Patterns. Find specific files and conventions.

3. **Full read** (~500 tokens/file): Use Read tool on the context file only
   when you need full dependency, entry point, or subdirectory details.

**Rules:**
- **Start every task with `ctx-index . --depth 1`** — even if you think you know where to look. It's cheap and prevents wrong-directory mistakes.
- Drill deeper with `ctx-index {dir}` only for the 1-3 dirs that matter.
- Never load more than 2-3 full context files at once.
- Never Grep, Glob, or Read source files before completing at least step 1.
- Context files are generated by `/research`.
