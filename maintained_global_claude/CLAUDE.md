...
- **The orchestrator's idle time belongs to the workers' critical path.** While workers run, never just wait. Keep a watchful eye on every worker's actual state (its worktree, its builds, its gates — not just its messages), anticipate what it will block on next, and clear that before it gets there: warm caches, prepare the next wave, pre-write briefs, free resources, re-sequence work so more of it runs in parallel, and pull anything off a worker's plate that someone else could do sooner. What the right intervention is depends on the project; the question is the same every turn: "what would make the wall-clock shorter right now?" — not "is anyone waiting on me?".
- **Fix failures as they appear, not when the run ends.** A long gate is a stream of verdicts, not one verdict: the moment a failure lands in the log, start fixing it (in a separate warm worktree so the gated tree stays untouched), and route each further failure the run surfaces to whoever is already fixing that class. Done right this is always faster than waiting for the summary line. The final gate still runs once, on the last SHA.
...
- **DEFAULT LANE: pre-dev integration whenever 2+ issues are in flight in one repo (2+ agents, or one session batching issues) — one gate per WAVE, never per issue/agent; red is delegated back to the owning agent.** The orchestrator branches `pre-dev` from `dev` before dispatch (a second concurrent wave gets `pre-dev2`, `pre-dev3`, … — same rules, own gate, own PR); workers never run gates or open PRs and `git merge --no-ff` into `pre-dev` when done; the orchestrator runs ONE `queue just ci-fast` on `pre-dev`, routes red to the owning worker, and on green opens ONE PR `pre-dev → dev`, then deletes every wave branch and worktree, local and remote. Hook-enforced: while `refs/heads/pre-dev` exists, gates are denied off `pre-dev`. Full mechanics: the "Concurrent lane" in `.agent-workflow/AGENT_WORKFLOW.md`.
