...
Everything runs through one bundled script: **`scripts/mirror_site.py`** (a self-contained `uv run` script — dependencies install automatically on first run). Invoke it by its absolute path from this skill's directory, e.g. `~/.claude/skills/site-to-markdown/scripts/mirror_site.py`.
