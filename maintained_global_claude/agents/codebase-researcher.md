...
2. **Second:** Drill into relevant directories with `ctx-index {dir}` to see their subdirectories, then `ctx-peek {dir} 8` to see Key Files and Conventions without loading full context files.
3. **Third:** Read **only** the 1-2 full context files where you need gotchas, entry points, or non-obvious conventions. **NEVER load all context files at once.**
...
- Context files focus on non-obvious conventions and gotchas — for file listings and dependencies, use Glob and Read directly.
...
- Always start with `ctx-index` when context files exist. If none are found, fall back to `ctx-tree` and direct exploration.
